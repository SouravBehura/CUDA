...
__global__ void assign_weights(double *d, int size, int no_neurons)
{
	int i = threadIdx.x;
	int j = blockIdx.x;
    int index = (j * size) + i;
    int total = size * no_neurons;
	int dev = total;
	total = total * 0.5;
	double sub = total - index;
	if(sub < 0)
	{
		sub = sub * (-1);
	}
...
{
	int i = threadIdx.x;
	activation[i] = pow(2.72, (double)activation[i]);
...
public:
	...
	neuron_layer(int no_neuron,neuron_layer prev_layer)
	{
		...
		assign_weights<<<neurons,prev_neurons>>>(dev_weights,prev_neurons,neurons);
...
int main()
{
	...
	neuron_layer output_layer(10, hidden_layer_2);
	...
	if (dec == 0)
	{
		...
		for (int x = 0; x <= 0; x++)
		{
			...
			cout << "0";
			hidden_layer_2.back_propagate(output_layer);
			cout << "1";
